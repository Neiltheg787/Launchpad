...
Return ONLY valid JSON matching this EXACT shape:
{
  ...
  "theme": {
    ...
    "inkDimColor": string (hex, secondary text),
    "accentColor": string (hex, the brand accent — NOT #38BDF8 unless the idea is truly cyberpunk),
...
async function renderPptx(
  ...
  pptx.author = 'Launchpad'
  pptx.company = 'Launchpad'
  ...
  cover.addText(
    [{ text: `LAUNCHPAD · ${theme.moodDescriptor.toUpperCase()}`, options: { color: ACCENT, fontSize: 11, bold: true, fontFace: FONT_B } }],
  ...
  for (const spec of copy.slides) {
    ...
    s.addText(
      [{ text: `LAUNCHPAD // ${theme.moodDescriptor.toUpperCase()}`, options: { color: INK_DIM, fontSize: 9, fontFace: FONT_B } }],
