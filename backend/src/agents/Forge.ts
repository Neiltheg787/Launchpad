...
_Scaffolded by [Launchpad](https://github.com/${owner}/${repo})._
`
}
...
const styles: Record<string, any> = {
  ...
  status: (ok: boolean) => ({ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '6px 14px', borderRadius: 20, fontSize: 13, fontFamily: 'JetBrains Mono, monospace', background: ok ? 'rgba(56,189,248,0.1)' : 'rgba(239,68,68,0.1)', color: ok ? colors.low : colors.high, border: \`1px solid \${ok ? 'rgba(56,189,248,0.25)' : 'rgba(239,68,68,0.25)'}\` }),
  ...
  return (
    <div style={styles.container}>
      <header style={styles.header}>
        <div style={styles.tag}>Launchpad Scaffold</div>
