/**
 * Pivot Engine route — runs the real Pivot agent against a report.
 *   POST /api/pivot/generate { reportId } → 5 ranked pivot ideas
 */
import { Router } from 'express'
import { db } from '../db/index.js'
import { runPivot } from '../agents/Pivot.js'
...
router.post('/generate', async (req, res, next) => {
  try {
    ...
    if (!r.scout_output || !r.atlas_output) {
      return res.status(400).json({ error: 'Report not ready (Scout + Atlas required)' })
    }
    const founder = await db.getFounder(req.founderId!)
    const dna = dnaFromFounder(founder)
    const out = await runPivot(
      r.ideaText,
