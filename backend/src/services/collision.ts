 ...
 *   - Internal DB: every idea submitted to Launchpad by other founders
 *
...
function buildSummary(r: Omit<CollisionReport, 'summary' | 'score'>, score: number): string {
  ...
  if (r.breakdown.internalIdeas > 0) parts.push(`${r.breakdown.internalIdeas} other founder${r.breakdown.internalIdeas === 1 ? '' : 's'} on Launchpad`)
