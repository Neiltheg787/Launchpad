/**
 * PDF generation service for Launchpad reports.
 *
...
function drawHeader(doc: PDFKit.PDFDocument, startupName: string, subtitle: string) {
  ...
  doc
    ...
    .fontSize(22)
    .fillColor(WHITE)
    .text(startupName || 'Launchpad Report', 50, 25, { width: doc.page.width - 100 })
...
function addPageNumbers(doc: PDFKit.PDFDocument) {
  ...
  for (let i = range.start; i < range.start + range.count; i++) {
    ...
    doc
      ...
      .fillColor('#888888')
      .text(
        `Page ${i + 1} of ${range.count}  ·  Launchpad`,
        50,
        doc.page.height - 40,
...
export async function generateValidationReport(reportId: string, report: any): Promise<string> {
  ...
  drawHeader(doc, startupName, 'Full Validation Report  ·  Launchpad')
...
export async function generateMarketResearch(reportId: string, report: any): Promise<string> {
  ...
  drawHeader(doc, startupName, 'Market Research Report  ·  Launchpad')
