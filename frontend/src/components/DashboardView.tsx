...
function QuickStats({
  ...
  const stats = [
    { label: 'Validation', value: validationScore ?? '—', max: '/10', color: '#38BDF8' },
...
function Header({
  ...
  const scoreColor = displayScore >= 7 ? '#38BDF8' : displayScore >= 5 ? '#FBBF24' : '#FB7185'
  return (
    <Glow>
    ...
    <div className="mt-8">
      ...
      <div className="grid md:grid-cols-2 gap-3">
        {competitors.slice(0, 8).map((c, i) => (
          <div key={i} className="border p-4" style={{ borderColor: 'rgba(56,189,248,0.15)' }}>
...
function CollisionPanel({ collision }: { collision: CollisionReport }) {
  const color =
    collision.score >= 70 ? '#FB7185' : collision.score >= 40 ? '#FBBF24' : '#38BDF8'
  return (
    <div className="mt-8">
      ...
      {collision.internalIdeas.length > 0 && (
        <div>
          <div className="font-mono text-[10px] uppercase tracking-[0.15em] text-muted mb-2">
            › other Launchpad founders building here
          </div>
          <div className="space-y-2">
    ...
    <div>
      ...
      <ResponsiveContainer width="100%" height={Math.max(200, labelData.length * 36 + 40)}>
        <BarChart data={labelData} layout="vertical" margin={{ left: 160, right: 20, top: 10, bottom: 10 }}>
          ...
          />
          <Tooltip
            contentStyle={{ background: '#1a1d24', border: '1px solid rgba(56,189,248,0.2)', fontSize: 11 }}
...
function DeckSlides({ deck, reportId }: { deck: DeckOut; reportId: string }) {
  ...
  const accentSoft = theme.accentSoftColor || 'rgba(56,189,248,0.25)'
      ...
      <div className="p-8">
        ...
        <div className="space-y-3">
          {displayList.map((inv) => {
            ...
            return (
              <div key={inv.id} className="border" style={{ borderColor: 'rgba(56,189,248,0.18)' }}>
                <div className="p-5">
                  ...
                  {eng && eng.stages.length > 0 && (
                    <div className="flex items-center gap-1.5 mb-3">
                      {eng.stages
                        ...
                        .map((s, i) => {
                        const stageColor =
                          s === 'clicked' ? { bg: 'rgba(56,189,248,0.12)', text: '#38BDF8' }
                          ...
                          : { bg: 'rgba(56,189,248,0.08)', text: 'rgba(56,189,248,0.7)' }
                        return (
                          <span
                            key={i}
                  ...
                  <div className="flex items-center gap-2 justify-end">
                    ...
                    <button
                      ...
                      style={{
                        background: sentState === 'sent' ? 'rgba(56,189,248,0.15)' : 'var(--color-charge)',
                ...
                {isOpen && (
                  ...
                  >
                    ...
                    <div
                      ...
                      style={{ borderColor: 'rgba(56,189,248,0.10)' }}
                    >
                      {inv.draftEmail?.body}
                    </div>
                  </div>
  ...
  return (
    <div className="flex items-center gap-4 pt-4">
      <div
        ...
        style={{ background: 'rgba(56,189,248,0.12)', color: 'var(--color-charge)' }}
      >
        {num}
      </div>
